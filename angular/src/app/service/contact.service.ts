...
export class ContactService {
  ...
  getSearchUrl1(field: string, value: number): string {
  ...
  searchPackage(field: string, value:number): Observable<Packages[]> {
  ...
  updateCustomer(customer: Customer): Observable<Customer> {
    return this.http.put(this.baseurl1, JSON.stringify(customer), this.getJsonContentTypeHeader()).pipe(
      map(data => data.json())
    );
  }
